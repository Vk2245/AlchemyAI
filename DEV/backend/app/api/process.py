            ...
            while True:
                ...
                if is_excel:
                    if progress == 100 and data:
                        ...
                        yield f"data: {json.dumps({'progress': 95, 'message': 'Saving results to database...', 'is_excel': True})}\n\n"
                        
                        # Keep all clusters exactly as generated by the LLM
                        ...
                        categories_to_research = [c for c in final_grouped_data.keys() if c not in ("Miscellaneous Categories", "Failed", "Uncategorized")]
                        
                        import concurrent.futures
                        
                        def _do_research(cat_name):
                            """Run web research for a single category (in thread)."""
                            try:
                                return cat_name, research_category(cat_name)
                            except Exception:
